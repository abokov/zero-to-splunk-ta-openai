...
### Output
This is what you should get as an outcome ( note: you may previous project name there :-))

```sh
(.venv) abokov@ABOKOV-M-X3L5 zero-to-splunk-ta-openai % python src/main.py --spec ./examples/test_api.md --output ./build/MyCustomTA
📊 Analyzing API Spec: 142 tokens (Est. Input Cost: $0.00071)
🚀 Calling OpenAI API...
☁️  Using Azure OpenAI Service...
📝 Rendering Splunk .conf files...
🐍 Writing Python collection script...
✅ Success! Splunk TA generated at: /Users/abokov/code/zero-to-splunk-ta-openai/build/MyCustomTA
(.venv) abokov@ABOKOV-M-X3L5 zero-to-splunk-ta-openai % 
```



/build/Custom_Splunk_TA/bin/weather_current.py
```python
import requests
import json
...
def fetch_current_weather(api_key, location, units='metric', timeout=30):
    headers = {
        'X-API-Key': api_key,
...
if __name__ == '__main__':
    # Example usage:
    # fetch_current_weather(api_key='YOUR_API_KEY', location='San Francisco', units='metric')
    pass%
```

build/Custom_Splunk_TA/default/inputs.conf
```plain
...
build/Custom_Splunk_TA/default/props.conf
```plain
[weather:current]
...
category = Custom
pulldown_type = 1
description = Auto-generated TA for weather:current%
```
